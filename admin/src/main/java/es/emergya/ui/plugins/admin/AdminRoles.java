...
public class AdminRoles extends Option {
	...
	public AdminRoles() {
		super("", PluginType.getType("ADMIN"), 3, //$NON-NLS-1$
				"subtab_icon_roles", null); //$NON-NLS-1$
		super.setTitle(i18n.getString("Roles.roles"));
		roles = new AdminPanel(
				i18n.getString("admin.roles.titulo"), LogicConstants.getIcon(ICON), this); //$NON-NLS-1$
		...
		SummaryAction action = new SummaryAction(r) {
			...
			protected JFrame getSummaryDialog() {
				...
				if (isNew) {
					...
					titulo = i18n
							.getString("admin.roles.tituloVentana.existente"); //$NON-NLS-1$
					...
					public void actionPerformed(ActionEvent e) {
						if (textfieldCabecera.getText().trim().isEmpty()) {
							JOptionPane.showMessageDialog(super.frame,
									i18n.getString("admin.roles13")); //$NON-NLS-1$
						} else if (isNew
								&& RolConsultas.alreadyExists(textfieldCabecera
										.getText().trim())) {
							JOptionPane.showMessageDialog(super.frame,
									i18n.getString("admin.roles14")); //$NON-NLS-1$
						} else if (textfieldCabecera.getText().isEmpty()) {
							JOptionPane.showMessageDialog(super.frame,
									i18n.getString("admin.roles15")); //$NON-NLS-1$
						} else if (cambios) {
							int i = JOptionPane
									.showConfirmDialog(
											super.frame,
											i18n.getString("admin.roles16"), i18n.getString("admin.roles17"), //$NON-NLS-1$ //$NON-NLS-2$
				...
				d = generateIconDialog(
						...
						right_label,
						guardar,
						LogicConstants.getIcon(i18n.getString("admin.roles19")),
			...
			protected boolean delete(boolean show_alert) {
				...
				if (!delete && show_alert) {
					JOptionPane.showMessageDialog(
							AdminRoles.this,
							i18n.getString("admin.roles23") //$NON-NLS-1$
									+ this.target.getNombre()
									+ i18n.getString("admin.roles24")); //$NON-NLS-1$
				}
				return delete;
			}
		};
